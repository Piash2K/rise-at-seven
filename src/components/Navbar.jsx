...
const Navbar = () => {
  ...
  const [hideHeaderBackground, setHideHeaderBackground] = useState(false);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setHideHeaderBackground(y < 40);
      setHideAnnouncementBar(y > 40);
    };
  ...
  const moveHover = (e) => {
    ...
    hoverRef.current.style.transform = `translateX(${left}px)`;
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 px-4">
      <div
        className={`w-full flex items-center justify-between relative z-20 px-4 transition
        lg:px-3 lg:rounded-full
        ${hideAnnouncementBar ? "translate-y-0" : "translate-y-12"}
        ${!hideHeaderBackground ? " backdrop-blur-lg" : ""}
        `}
      >
        <a
          href="/"
        ...
        <div
          ref={navRef}
          className="relative ml-10 hidden lg:inline-flex group"
        >
          <div
            ref={hoverRef}
            className="absolute h-full rounded-full  transition-all duration-300 opacity-0 pointer-events-none"
          />

          {links.map((item) => (
            <div key={item.label} className="relative z-10">
              <a
                onMouseEnter={moveHover}
                className={`inline-flex px-4 py-1 text-sm font-medium transition ${
                  hideHeaderBackground ? "text-white" : "text-gray-900"
                }`}
                href="#"
              >
                {item.label}
                ...
                {item.badge && (
                  <div className="absolute top-0 right-0 -translate-y-2.5 px-1.5 py-0.5 text-xs rounded-full bg-green-300 text-gray-900">
                    {item.badge}
                  </div>
                )}
              </a>
        ...
        <div className="hidden lg:inline-flex">
          <a
            href="#"
            className={`px-6 py-3 rounded-3xl text-sm font-medium transition-all hover:rounded-xl ${
        ...
        <button
          ...
          className="lg:hidden w-12 h-8 flex items-center justify-center"
        >
          <div className="flex flex-col justify-between h-3 w-5">
            <span
              className={`h-0.5 w-full transition ${
                mobileMenu ? "rotate-45 translate-y-1" : ""
              } ${hideHeaderBackground ? "bg-white" : "bg-black"}`}
            />
            <span
              className={`h-0.5 w-full transition ${
                mobileMenu ? "-rotate-45 -translate-y-1" : ""
      ...
      {mobileMenu && (
        <div className="lg:hidden bg-white p-6 space-y-4">
          {links.map((item) => (
            <a key={item.label} className="block text-lg font-medium">
              {item.label}
            </a>
          ))}
        </div>
