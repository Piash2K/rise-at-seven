...
const Footer = () => {
      ...
      mediaQuery = ScrollTrigger.matchMedia({
        '(pointer: fine)': () => {
          backgroundAnimation = ScrollTrigger.create({
            ...
            onUpdate: (update) => {
              gsap.set(footerBackground, { height: `${update.progress * 100}%` });
            },
          });
          ...
          contentAnimation = gsap.to(footerContent, {
            opacity: 1,
            ease: 'none',
            scrollTrigger: { trigger: footer, start: 'top 80%', end: 'bottom 110%', scrub: 1 },
          });
          return () => {
  ...
  return (
    <section ref={footerRef} className="w-full py-0" id="footer">
      <div className="w-full px-2 pb-2">
        <div className="relative bg-black rounded-3xl overflow-hidden">
          {/* Background overlay */}
          <div ref={footerBackgroundRef} className="absolute top-0 left-0 w-full h-full flex">
          ...
          <div
            ...
            className="relative z-20 grid grid-cols-12 pt-20 pb-6 lg:py-10 px-4 md:px-7 gap-x-3 md:gap-x-5 gap-y-3 md:gap-y-7"
          >
            ...
            <div className="flex flex-col items-start justify-start col-span-12 mb-10 lg:mb-0 lg:col-span-4 gap-y-3 md:gap-y-5">
              <h2 className="text-white text-3xl/none xl:text-4xl/none font-sans-primary font-medium tracking-tight mb-4">
                Stay updated with Rise news
              </h2>

              {/* Newsletter Form with smooth arrow transition */}
              <form method="post" className="w-full relative">
                <input type="hidden" name="CRAFT_CSRF_TOKEN" value="soggMTBiREcE4uaDY-3E60Ma_cYb5nUtcKrT1gjgVFTRwQr-LCZ8gffXZXNiA3QSaYzU2QKvqLskQ4q-UdAAGhmZ6r1DlyZn5JFGiAFwNfQ=" />
                ...
                <input
                  ...
                  required
                  name="fields[email]"
                  className="appearance-none transition bg-neutral-700 rounded-full w-full text-white font-medium tracking-tight leading-none text-lg px-5 py-4 lg:text-xl lg:px-6 lg:py-5 placeholder:text-white/50 focus:outline-none focus:ring-3 focus:ring-white/15"
                ...
                <div className="absolute top-0 right-0 p-2 ">
                  <button
                    type="submit"
                    className="group relative size-9 lg:size-13 rounded-full flex items-center justify-center cursor-pointer transition-colors duration-300 bg-[#B7F3E4] hover:bg-white"
                  >
                    <div className="relative w-5 h-5 lg:w-6 lg:h-6 ">
                      {/* Up-right arrow (visible normally) */}
                      <svg
                        className="absolute inset-0 w-full h-full text-black transition-all duration-300  group-hover:opacity-0 group-hover:rotate-45"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      ...
                      {/* Down-right arrow (visible on hover) */}
                      <svg
                        className="absolute inset-0 w-full h-full text-black  transition-all duration-300 opacity-0 rotate-0 group-hover:opacity-100 group-hover:rotate-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 7L17 17M17 17V7M17 17H7" />
                      </svg>
                    </div>
                  </button>
                </div>
              ...
              {/* Social Links with black arrows */}
              <div className="flex gap-1 mt-2 flex-wrap">
                {[
                  { href: "https://www.facebook.com/riseatseven", icon: "fab fa-facebook-f" },
                  { href: "https://x.com/riseatseven", icon: "fab fa-x-twitter" },
                  { href: "https://www.linkedin.com/company/riseatseven/", icon: "fab fa-linkedin-in" },
                  { href: "https://www.youtube.com/channel/UCAjOP9BgpZPTgae-QT9HGCw", icon: "fab fa-youtube" },
                  { href: "https://www.tiktok.com/@riseatseven", icon: "fab fa-tiktok" },
                  { href: "https://www.instagram.com/riseatseven/", icon: "fab fa-instagram" },
                ].map((social) => (
                  <a
                    key={social.href}
                    href={social.href}
                    ...
                    className="inline-flex items-center gap-x-2 rounded-xl text-xs px-2 py-1 transition-all hover:rounded-sm bg-white text-black"
                  >
                    <i className={`${social.icon} text-black`}></i>
                    <svg
            ...
            {/* Navigation Columns - larger text */}
            <div className="flex justify-between col-span-12 flex-wrap md:flex-row md:col-span-11 lg:col-span-6 lg:col-start-6 gap-y-10">
              {/* Column 1 */}
              <div className="flex flex-col items-start gap-y-2 border-l border-white/20 pl-3 w-1/2 md:w-auto">
                {["Services", "Work", "About", "Culture", "Meet The Risers"].map((label, idx) => {
                  const href = idx === 0 ? "https://riseatseven.com/services/" : idx === 1 ? "https://riseatseven.com/work/" : idx === 2 ? "https://riseatseven.com/about/" : idx === 3 ? "https://riseatseven.com/culture/" : "https://riseatseven.com/meet-the-team/";
                  return (
                    <a key={label} href={href} className="group inline-flex text-white font-medium tracking-tight leading-tight text-xl lg:text-2xl hover:text-mint transition">
                      <div className="relative overflow-hidden">
                        <div className="transition group-hover:-translate-y-8">{label}</div>
                        <div className="transition absolute top-0 left-0 translate-y-8 group-hover:translate-y-0">{label}</div>
                      </div>
                    </a>
                  );
                })}
              ...
              {/* Column 2 */}
              <div className="flex flex-col items-start gap-y-2 border-l border-white/20 pl-3 w-1/2 md:w-auto">
                {["Testimonials", "Blog & Resources", "Webinars", "Careers"].map((label, idx) => {
                  const href = idx === 0 ? "https://riseatseven.com/testimonials/" : idx === 1 ? "https://riseatseven.com/blog/" : idx === 2 ? "https://riseatseven.com/webinars/" : "https://riseatseven.com/careers/";
                  return (
                    <a key={label} href={href} className="group inline-flex text-white font-medium tracking-tight leading-tight text-xl lg:text-2xl hover:text-mint transition">
                      <div className="relative overflow-hidden">
                        <div className="transition group-hover:-translate-y-8">{label}</div>
              ...
              <div className="flex flex-col items-start gap-y-2 border-l border-white/20 pl-3 w-1/2 md:w-auto">
                {[
                  ...
                  { label: "Contact", href: "https://riseatseven.com/contact/", external: false },
                ].map((item) => (
                  <a
                    key={item.label}
                    href={item.href}
                    ...
                    className="group inline-flex text-white font-medium tracking-tight leading-tight text-xl lg:text-2xl hover:text-mint transition"
                  >
                    <div className="relative overflow-hidden">
                      <div className="transition group-hover:-translate-y-8">{item.label}</div>
                      <div className="transition absolute top-0 left-0 translate-y-8 group-hover:translate-y-0">{item.label}</div>
                    </div>
                  </a>
                ))}
              </div>
            ...
            <div className="col-span-12 flex justify-between flex-col mt-10 items-end md:flex-row lg:items-center lg:mt-0">
              <div className="flex gap-x-2 gap-y-2 flex-wrap items-center md:gap-3">
                <span className="text-white font-light text-1xs md:text-xs">© 2025 Rise at Seven Ltd. All rights reserved</span>
                <span className="w-1 h-1 rounded-full bg-white hidden md:inline-block"></span>
                <span className="text-white font-light text-1xs md:text-xs">Company Number 11955187</span>
                <span className="w-1 h-1 rounded-full bg-white hidden md:inline-block"></span>
                <span className="text-white font-light text-1xs md:text-xs">VAT Registered GB 322402945</span>
                <span className="w-1 h-1 rounded-full bg-white hidden md:inline-block"></span>
                <a href="https://riseatseven.com/privacy-policy/" className="text-white font-light text-1xs md:text-xs hover:underline">Privacy Policy</a>
                <span className="w-1 h-1 rounded-full bg-white hidden md:inline-block"></span>
                <a href="https://riseatseven.com/terms-conditions/" className="text-white font-light text-1xs md:text-xs hover:underline">Terms &amp; conditions</a>
              </div>
              <div className="w-full mt-1 md:ml-auto md:text-right lg:mt-0 lg:w-auto">
                <a href="https://madebyshape.co.uk" target="_blank" rel="noopener noreferrer" className="text-white font-light text-1xs md:text-xs hover:underline">Website MadeByShape</a>
              </div>
            </div>
          </div>
        </div>
